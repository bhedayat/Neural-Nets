...
hidden_layer = 120; 
% omega1 = normrnd(0,1/sqrt(size(X,2)), [size(X,2)+1,hidden_layer]);
% omega2 = normrnd(0,1/sqrt(hidden_layer), [hidden_layer+1,num_labels]);
omega1 = randn(size(X,2)+1,hidden_layer);
omega2 = randn(hidden_layer+1,num_labels);
...
%% Back Propagation
    ...
    delta2 = target_r-g2;
    delta1 = (delta2*omega2').*tanhGrad([ones(batch,1) a1]);
    % Regularization
    reg1 = lambda*[zeros(size(omega1, 1), 1) omega1(:, 2:end)];
    reg2 = lambda*[zeros(size(omega2, 1), 1) omega2(:, 2:end)];
    
    grad1 = delta1(:,2:end)'*X_r - reg1'  + momentum*diff1;
    grad2 = delta2'*g1 - reg2' + momentum*diff2';
...
%     error(i) = -sum(sum(target_train'*log(tg2),2));
    ...
    %% Checking validation
    v1 = 1.7159*tanh((2/3)*X_val*omega1);
...
%     errorv(i) = -sum(sum(target(50001:end,:)'*log(v2),2));
    ...
    %% Testing accuracy 
    p1 = 1.7159*tanh((2/3)*X_test*omega1);
